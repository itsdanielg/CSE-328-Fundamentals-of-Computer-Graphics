...
void drawTriangle(float *v1, float *v2, float *v3, float offset) {
	...
	float three[3] = {v3[0] + offset, v3[1], v3[2]};
	glNormal3fv(one);
	glVertex3fv(one);
	glNormal3fv(two);
	glVertex3fv(two);
...
void subdivide(float *v1, float *v2, float *v3, long depth, float offset) {
	...
	normalize(v23);
	normalize(v31);
	subdivide(v1, v12, v31, depth-1, offset);
	subdivide(v2, v23, v12, depth-1, offset);
	subdivide(v3, v31, v23, depth-1, offset);
